  ...
  return (
    <Card className="w-full max-w-2xl mx-auto mt-8">
      <CardHeader>
        ...
        <CardDescription>Enter your meeting summary and we'll extract tasks for your Monday.com board.</CardDescription>
      </CardHeader>
      <CardContent>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-2">
            <Label htmlFor="boardId">Monday.com Board ID</Label>
            <Input
              id="boardId"
              value={boardId}
            ...
            {tasks.map((task, index) => (
              <Card key={index}>
                <CardHeader>
                  ...
                  {task.assignee && <CardDescription>Assignee: {task.assignee}</CardDescription>}
                </CardHeader>
                <CardContent>
                  <p>{task.description}</p>
                  {task.dueDate && <p className="text-sm text-muted-foreground mt-2">Due: {task.dueDate}</p>}
